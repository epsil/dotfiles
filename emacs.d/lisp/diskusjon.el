...
(defvar diskusjon-mode-hook nil)
...
\\(\\(?:.\\|\n\\)*?\\)\\[/url\\]"
  ...
  (run-hooks 'diskusjon-mode-hook))

(define-minor-mode diskusjon-punctuation-mode
  "Minor mode for tegnsetting."
  :keymap (make-sparse-keymap)
  (run-hooks 'diskusjon-punctuation-mode-hook))

(defun turn-on-diskusjon-punctuation-mode ()
  "Aktiver diskusjon-punctuation-mode."
  (interactive)
  (diskusjon-punctuation-mode 1))

(define-key diskusjon-punctuation-mode-map "'"  'diskusjon-single-quote)
(define-key diskusjon-punctuation-mode-map "\"" 'diskusjon-double-quote)
;; (define-key diskusjon-punctuation-mode-map "`"  'diskusjon-code-tag)
;; (define-key diskusjon-punctuation-mode-map "´"  'diskusjon-del-tag)
(define-key diskusjon-punctuation-mode-map "\C-cl" 'diskusjon-switch-lang)
(define-key diskusjon-punctuation-mode-map (kbd "RET") 'diskusjon-ret)
...
(defun diskusjon-switch-lang ()
  "Bytte mellom språk."
  (interactive)
  (if (equal diskusjon-lang "norsk")
      (diskusjon-lang "english")
    (diskusjon-lang "norsk")))
...
(defun diskusjon-quotes ()
  "Doble og enkle anførselstegn for ulike språk."
  (if diskusjon-ascii
      diskusjon-ascii-quotes
    diskusjon-quotes))
...
(defun diskusjon-insert-tag (tag)
  "Sett inn <tag>...</tag>."
  (interactive)
  (let ((open (format "<%s>" tag))
        (close (format "</%s>" tag)))
    (cond
     ((region-active-p)
      (skeleton-insert `(nil ,open _ ,close) -1))
     ((and (save-excursion
             (re-search-backward
              (format "%s\\|%s"
                      (regexp-quote open)
                      (regexp-quote close)) nil t))
           (equal (match-string 0) open))
      (insert close))
     (t
      (insert open)))))

(defun diskusjon-del-tag ()
  "Sett inn <del>...</del>."
  (interactive)
  (if (not (region-active-p))
      (diskusjon-insert-tag "del")
    (let ((beg (region-beginning))
          (end (region-end))
          (comment-start "<del>")
          (comment-end "</del>")
      ...
      (save-excursion
        (goto-char beg)
        (beginning-of-line)
        (while (re-search-forward "^<del>\\([*#]+\\) " end t)
          (replace-match "\\1 <del>" nil nil))))))

(defun diskusjon-strike ()
  "Sett inn ~~...~~."
  (interactive)
  (if (not (region-active-p))
...
;;           (replace-match "\\1\\2<s>" nil nil))))))

(defun diskusjon-code-tag ()
  "Sett inn <code>...</code>."
  (interactive)
  (cond
   ((and (region-active-p)
         (save-excursion
           ...
           (and (bolp) (eolp))))
    (skeleton-insert `(nil "<pre>\n" _ "</pre>\n") -1))
   (t
    (diskusjon-insert-tag "code"))))
...
(defun diskusjon-ret ()
  ...
  (let ((prefix (save-excursion
    ...
    (when (looking-at "[]= )}\"']+$")
      (end-of-line))
    (cond
     ((and prefix (looking-back (concat "^" (regexp-quote prefix))))
      (kill-whole-line 0))
     (t
      (newline)
      (when prefix
...
;; (define-skeleton diskusjon-strike
...
(defun diskusjon-tt (beg end)
  ...
  (save-excursion
    ...
    (replace-regexp "‘" "'" nil beg end)
    (replace-regexp "’" "'" nil beg end))
  (skeleton-insert '(nil "[font=\"Courier New\"]" _ "[/font]"))
...
(define-key diskusjon-mode-map "`"            'diskusjon-tt)
(define-key diskusjon-mode-map "\C-cl"        'diskusjon-switch-lang)
